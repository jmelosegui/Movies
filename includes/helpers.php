...
function extractImageUrl($rawValue = ''){
	if($rawValue){
		$offset = strpos($rawValue, "http://");
		return str_replace("w500", "w92", substr($rawValue, $offset,(strpos($rawValue, '>') - $offset - 1 ))) ;
	}
	else{
		return "http://' . $_SERVER[HTTP_HOST] .'/assets/images/noimg.png";
	}
}
